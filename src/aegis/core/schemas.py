...
from pandera.pandas import (
    ...
    Check,
    DataFrameSchema,
)  # Thư viện định nghĩa cấu trúc dữ liệu dạng bảng và các điều kiện ràng buộc
from typing import (
    Dict,
    Any,
    TypedDict,
...
SignalBarSchema = DataFrameSchema(
    columns={
        "bar_idx": Column(
            int, Check.ge(0), unique=True
        ),  # Chỉ số thứ tự tuyệt đối của nến (>= 0, duy nhất)
        "symbol": Column(str),  # Ký hiệu mã giao dịch (ví dụ: BTCUSDT)
        "timestamp_ms": Column(
            int, Check.ge(0)
        ),  # Thời gian đóng nến tính bằng mili giây epoch (knowledge_time)
        ...
        "ofi": Column(
            float, Check.in_range(-1.0, 1.0)
        ),  # Chỉ số mất cân bằng dòng lệnh chuẩn hóa Order Flow Imbalance [-1.0, 1.0]
        "tick_count": Column(
            int, Check.ge(0)
        ),  # Số lượng tick cấu thành nên cây nến Dollar-Volume (>= 0)
        "is_toxic_flag": Column(
            bool
        ),  # Cờ báo hiệu nến có thanh khoản độc hại (tick_count < 0.5 * median)
        "is_tail_event": Column(
            bool
        ),  # Cờ báo hiệu sự kiện đuôi thanh khoản đột biến (Black Swan / Tail Event)
        "insufficient_history": Column(
            bool
        ),  # Cờ báo hiệu nến thuộc giai đoạn khởi động chưa đủ dữ liệu lịch sử
        "trend_score": Column(
            float, nullable=True
        ),  # Điểm số xu hướng từ tín hiệu sơ cấp (cho phép Null khi khởi động)
        "p_trend": Column(
            float, Check.in_range(0.0, 1.0), nullable=True
        ),  # Xác suất xu hướng HMM [0, 1] (cho phép Null)
        "p_chop": Column(
            float, Check.in_range(0.0, 1.0), nullable=True
        ),  # Xác suất đi ngang HMM [0, 1] (cho phép Null)
        "atr_14": Column(
            float, Check.ge(0), nullable=True
        ),  # Chỉ số biến động trung bình ATR 14 chu kỳ (>= 0, cho phép Null)
        "hurst_value": Column(
            float, Check.in_range(0.0, 1.0), nullable=True
        ),  # Chỉ số số mũ Hurst GHE [0, 1] (cho phép Null)
        "d_star_used": Column(
            float, Check.in_range(0.0, 1.0), nullable=True
        ),  # Bậc vi phân từng phần FFD tối ưu [0, 1] (cho phép Null)
    },
    checks=[
        Check(
            check_ohlc_logic, name="check_ohlc_logic"
        ),  # Gắn kiểm tra hợp đồng logic nến OHLC
        Check(
            check_insufficient_history_nulls, name="check_insufficient_history_nulls"
        ),  # Gắn kiểm tra logic Null khi insufficient_history=True
    ],
    strict=True,
...
TradeRecordSchema = DataFrameSchema(
    columns={
        "schema_version": Column(
            str, Check.eq("1.0.0")
        ),  # Phiên bản của cấu trúc dữ liệu giao dịch (khóa cứng "1.0.0")
        "dataset_manifest_hash": Column(
            str
        ),  # Mã băm SHA-256 của chuỗi nến sinh ra giao dịch này
        "fold_id": Column(
            str, nullable=True
        ),  # ID của fold kiểm định CPCV (ví dụ: "fold_0", cho phép Null khi chạy live)
        "symbol": Column(str),  # Ký hiệu mã giao dịch
        "entry_idx": Column(
            int, Check.ge(0)
        ),  # Chỉ số thứ tự tuyệt đối của nến tại thời điểm vào lệnh (>= 0)
        "entry_price": Column(float, Check.gt(0)),  # Giá thực tế vào lệnh (> 0)
        "p_i": Column(
            float, Check.in_range(0.0, 1.0)
        ),  # Xác suất xu hướng tại thời điểm vào lệnh [0, 1]
        "p_chop_i": Column(
            float, Check.in_range(0.0, 1.0)
        ),  # Xác suất đi ngang tại thời điểm vào lệnh [0, 1]
        "mode": Column(
            str, Check.isin(["follow", "fade"])
        ),  # Chế độ vào lệnh: follow (theo xu hướng) hoặc fade (đảo chiều)
        "side": Column(
            int, Check.isin([-1, 1])
        ),  # Chiều thực tế của lệnh: +1 (Long) hoặc -1 (Short)
        "sl_initial": Column(float, Check.gt(0)),  # Giá cắt lỗ ban đầu (> 0)
        "size_notional": Column(
            float, Check.gt(0)
        ),  # Quy mô danh nghĩa của lệnh giao dịch (> 0)
        "exit_idx_relative": Column(
            int, Check.ge(0)
        ),  # Khoảng cách bar tương đối từ lúc vào lệnh đến lúc thoát lệnh (>= 0)
        "exit_idx_absolute": Column(
            int, Check.ge(0)
        ),  # Chỉ số bar tuyệt đối khi thoát lệnh (= entry_idx + 1 + exit_idx_relative)
        "exit_reason": Column(
            str, Check.isin(["SL", "TRAIL", "REGIME_FLIP", "TIME_STOP"])
        ),  # Lý do thoát lệnh chuẩn hóa
        "fill_price_exit": Column(
            float, Check.gt(0)
        ),  # Giá khớp lệnh thoát lệnh thực tế tra cứu tại exit_idx_absolute (> 0)
        "boundary_truncated": Column(
            bool
        ),  # Cờ báo hiệu giao dịch bị cắt ngắn do hết dữ liệu hoặc chạm biên fold
        "fee_entry": Column(float, Check.ge(0)),  # Phí giao dịch vào lệnh (>= 0)
        "fee_exit": Column(float, Check.ge(0)),  # Phí giao dịch thoát lệnh (>= 0)
        "funding_accrued": Column(
            float
        ),  # Tổng chi phí/lợi tức lãi qua đêm (funding rate) tích lũy
        "gross_pnl": Column(float),  # Lợi nhuận gộp chưa trừ phí
        "realized_return": Column(
            float
        ),  # Lợi nhuận ròng thực tế (Realized Return) sau phí và funding
    },
    checks=[
        Check(
            check_absolute_index_logic, name="check_absolute_index_logic"
        )  # Gắn kiểm tra hợp đồng logic exit_idx_absolute
    ],
    strict=True,
...
def compute_dataset_manifest_hash(
    bar_df: pd.DataFrame, generation_params: Dict[str, Any]
) -> str:
    """
...
def assert_trade_records_match_bar_version(
    trade_df: pd.DataFrame, expected_manifest_hash: str
):
    """
    ...
    assert (
        len(unique_hashes) == 1
    ), "FATAL: Lẫn lộn nhiều phiên bản dataset trong cùng một Trade DataFrame."
