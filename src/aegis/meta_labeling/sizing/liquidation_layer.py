...
# ============================================================================
def compute_liquidation_price(entry_price: float, side: int, leverage: float, maintenance_margin_rate: float) -> float:
    """
    ...
    """
    if side not in (1, -1):
        raise ValueError(f"Lỗi hải quan v11.9 (compute_liquidation_price): side bắt buộc phải là +1 (Long) hoặc -1 (Short), nhận {side}")

    if not isinstance(entry_price, (int, float)) or math.isnan(entry_price) or math.isinf(entry_price) or entry_price <= 0:
        raise ValueError(f"Lỗi hải quan v11.9: entry_price phải là số dương hợp lệ (> 0), nhận {entry_price}")

    if not isinstance(leverage, (int, float)) or math.isnan(leverage) or math.isinf(leverage) or leverage < 1.0:
        raise ValueError(f"Lỗi hải quan v11.9: đòn bẩy leverage phải >= 1.0 và hợp lệ, nhận {leverage}")

    if not isinstance(maintenance_margin_rate, (int, float)) or math.isnan(maintenance_margin_rate) or math.isinf(maintenance_margin_rate) or not (0.0 <= maintenance_margin_rate < 1.0):
        raise ValueError(f"Lỗi hải quan v11.9: maintenance_margin_rate phải nằm trong đoạn [0.0, 1.0), nhận {maintenance_margin_rate}")
...
def validate_leverage_against_sl(
    entry_price: float, side: int, sl_initial: float,
    leverage: float, maintenance_margin_rate: float, safety_buffer_pct: float = 0.15
) -> Dict[str, Any]:
    """
    ...
    """
    if not isinstance(safety_buffer_pct, (int, float)) or math.isnan(safety_buffer_pct) or math.isinf(safety_buffer_pct) or not (0.0 <= safety_buffer_pct <= 0.9):
        raise ValueError(f"Lỗi hải quan v11.9: safety_buffer_pct chỉ được phép từ 0.0 đến 0.9 (0% - 90%), nhận {safety_buffer_pct}")

    if not isinstance(sl_initial, (int, float)) or math.isnan(sl_initial) or math.isinf(sl_initial) or sl_initial <= 0:
        raise ValueError(f"Lỗi hải quan v11.9: sl_initial phải là số dương > 0, nhận {sl_initial}")

    liq_price = compute_liquidation_price(entry_price, side, leverage, maintenance_margin_rate)
    ...
    if distance_to_sl <= 0:
        raise ValueError(f"Lỗi hải quan v11.9: Điểm Cắt Lỗ sl_initial ({sl_initial}) đặt sai chiều hoặc bằng Entry ({entry_price}) cho lệnh side={side}!")
...
def resolve_max_safe_leverage(
    entry_price: float, side: int, sl_initial: float,
    maintenance_margin_rate: float, safety_buffer_pct: float = 0.15, leverage_cap: float = 20.0
) -> float:
    """
    ...
    """
    if side not in (1, -1):
        raise ValueError(f"Lỗi hải quan v11.9 (resolve_max_safe_leverage): side bắt buộc phải là +1 hoặc -1, nhận {side}")

    if not isinstance(entry_price, (int, float)) or math.isnan(entry_price) or math.isinf(entry_price) or entry_price <= 0:
        raise ValueError(f"Lỗi hải quan v11.9: entry_price phải > 0, nhận {entry_price}")

    if not isinstance(sl_initial, (int, float)) or math.isnan(sl_initial) or math.isinf(sl_initial) or sl_initial <= 0:
        raise ValueError(f"Lỗi hải quan v11.9: sl_initial phải > 0, nhận {sl_initial}")

    if not isinstance(safety_buffer_pct, (int, float)) or math.isnan(safety_buffer_pct) or not (0.0 <= safety_buffer_pct <= 0.9):
        raise ValueError(f"Lỗi hải quan v11.9: safety_buffer_pct phải từ [0.0, 0.9], nhận {safety_buffer_pct}")

    if not isinstance(leverage_cap, (int, float)) or math.isnan(leverage_cap) or leverage_cap < 1.0:
        raise ValueError(f"Lỗi hải quan v11.9: leverage_cap phải >= 1.0, nhận {leverage_cap}")
...
def test_liquidation_layer_armor_plated():
    ...
    check_long = validate_leverage_against_sl(entry, 1, sl_long, lev, maint, safety_buffer_pct=0.15)
    assert check_long["is_safe"] is True, f"Long safe check failed: {check_long}"
    assert abs(check_long["liq_price"] - 80.5) < 1e-6, f"Sai giá thanh lý Long: {check_long['liq_price']}"
    
    # 2. Test case Short chuẩn
    # Liq = 100 * (1 + 0.2 - 0.005) = 119.5
    check_short = validate_leverage_against_sl(entry, -1, sl_short, lev, maint, safety_buffer_pct=0.15)
    assert check_short["is_safe"] is True, f"Short safe check failed: {check_short}"
    assert abs(check_short["liq_price"] - 119.5) < 1e-6, f"Sai giá thanh lý Short: {check_short['liq_price']}"
    
    # 3. Test giải closed-form đòn bẩy tối đa cho Long
    # denom = (0.10 / 0.85) + 0.005 = 0.117647 + 0.005 = 0.122647
    # L_max = 1 / 0.122647 = 8.153478
    max_lev_long = resolve_max_safe_leverage(entry, 1, sl_long, maint, safety_buffer_pct=0.15, leverage_cap=20.0)
    ...
    try:
        resolve_max_safe_leverage(entry, 1, 105.0, maint) # Long nhưng SL ở 105 (> entry)
    ...
    try:
        validate_leverage_against_sl(entry, 1, sl_long, lev, maint, safety_buffer_pct=1.2)
        assert False, "Lỗi rò rỉ: safety_buffer_pct = 1.2 không bị chặn!"
    except ValueError as e:
        assert "chỉ được phép từ 0.0 đến 0.9" in str(e)

    print("✅ [v11.9] Liquidation Layer PASSED! (Tính chính xác tuyệt đối & Chống 100% chia cho 0 / SL ngược / đòn bẩy rác)")
