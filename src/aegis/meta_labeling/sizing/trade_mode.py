...
def classify_trade_mode(p_i: float, p_chop_i: float, fade_enabled: bool,
    ...
    if not isinstance(p_i, (int, float)) or math.isnan(p_i) or math.isinf(p_i) or not (0.0 <= p_i <= 1.0):
        raise ValueError(f"Lỗi hải quan B-1-2: p_i phải là số thực hợp lệ trong đoạn [0, 1], nhận {p_i}")
        
    if not isinstance(p_chop_i, (int, float)) or math.isnan(p_chop_i) or math.isinf(p_chop_i) or not (0.0 <= p_chop_i <= 1.0):
        raise ValueError(f"Lỗi hải quan B-1-2: p_chop_i phải là số thực hợp lệ trong đoạn [0, 1], nhận {p_chop_i}")
...
def test_b_1_2_trade_mode():
    ...
    """
    # 1. Nhánh Follow
    assert classify_trade_mode(0.5, 0.4, True) == "follow", "Lỗi: p=0.5 ngay tại biên phải là follow"
    assert classify_trade_mode(0.6, 0.4, True) == "follow", "Lỗi: p >= 0.5 phải là follow"
    
    # 2. Vùng Deadzone (Đứng ngoài)
    assert classify_trade_mode(0.3, 0.8, True) == "none", "Lỗi: p nằm trong [0.2, 0.5) phải là none (Deadzone)"
    assert classify_trade_mode(0.1, 0.5, True) == "none", "Lỗi: p < 0.2 nhưng p_chop <= 0.6 phải bị khóa (none)"
    
    # 3. Nhánh Fade
    assert classify_trade_mode(0.1, 0.7, True) == "fade", "Lỗi: Đủ điều kiện Fade nhưng không kích hoạt"
    
    # 4. Fade bị Disable
    assert classify_trade_mode(0.1, 0.7, False) == "none", "Lỗi: Fade đang bị disable thì không được kích hoạt"
    ...
    for p, p_chop, fe in invalid_inputs:
        try:
            ...
            assert False, f"Lỗi rò rỉ: Input dị thường ({p}, {p_chop}) lọt qua hải quan mà không báo lỗi!"
        except ValueError:
            pass
            
    print("✅ [TASK B-1-2] classify_trade_mode PASSED! (Xử lý mượt mà điểm mù & chống 100% rác NaN/Out-of-bounds)")
